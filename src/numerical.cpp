...
void Numerical::ludcmp(float *A, int n, uint* indx, float d, float *fac)
{
    int i, imax=0, j, k;
    float big, temp;
    float *vv = new float[n];
    float *a = new float[n*n];
    memcpy(a, A, sizeof(float) * 6 * 6);
    for (i = 0; i < n; i++) {
        big = 0.0;
        for (j = 0; j < n; j++)
...
void Numerical::lubksb(float *a, int n, uint* indx, float *b, float *x)
{
    int i, ii = 0, ip, j;
    float sum;
...
float Numerical::pythag(float a, float b)
{
    float absa, absb;
    absa = fabs(a);
    absb = fabs(b);
    if (absa > absb) return absa * sqrtf(1.0f + sqrtf(absb / absa));
    else return (absb == 0.0f ? 0.0f : absb * sqrtf(1.0f + sqrtf(absa / absb)));
}

void Numerical::svdcmp(float *a, int m, int n, float *U, float *w, float *v)
{
    bool flag;
    int i, its, j, jj, k, l, nm;
    float anorm, c, f, g, h, s, scale, x, y, z, *rv1;
    rv1 = new float[n];
    memcpy(U, a, sizeof(float)*m*n);
    g = scale = anorm = 0.0;
    for (i = 0; i < n; i++) {
        l = i + 2;
        rv1[i] = scale * g;
...
void Numerical::absh3Initialize(float h, uint array_size)
{
    step_size = h;
    n = array_size;
    Y_next = new float[n];
...
float Numerical::absh3(float *Y, float *Yp, float t_current) {
    ...
    case 1:
        ...
        for (uint i = 0; i < n; i++) {
            Y_next[i] = Y[i] + step_size * Yp[i] / 4.0f;
        }
        // w(:,2) = Yp;
        for (uint i = 0; i < n; i++) {
            AW[i * 2 + 1] = Yp[i];
        ...
        t_next = t_current + step_size / 4.0f;
        break;
    case 2:
        ...
        for (uint i = 0; i < n; i++) {
            Y_next[i] = Y[i] + step_size * (3 * Yp[i] - AW1[i * 2 + 1]) / 8.0f;
        }
        // w1(:,1) = Yp;
        for (uint i = 0; i < n; i++) {
            AW1[i * 2] = Yp[i];
    ...
    case 3:
        ...
        for (uint i = 0; i < n; i++) {
            Y_next[i] = Y[i] + step_size * (23.0f*Yp[i] - 16.0f*AW1[i * 2] + 5.0f*AW1[i * 2 + 1]) / 48.0f;
        }
        // w1(:,2) = w1(:,1);
        for (uint i = 0; i < n; i++) {
            AW1[i * 2 + 1] = AW1[i * 2];
    ...
    case 5:
        ...
        for (uint i = 0; i < n; i++) {
            Y_next[i] = Y[i] + step_size * (23.0f*Yp[i] - 16.0f*AW1[i * 2] + 5.0f*AW1[i * 2 + 1]) / 24.0f;
        }
        // w(:,1) = Yp;
        for (uint i = 0; i < n; i++) {
            AW[i * 2] = Yp[i];
        ...
        t_next = t_current + step_size / 2.0f;
        break;
    case 6:
    ...
    case 7:
        ...
        for (uint i = 0; i < n; i++) {
            Y_next[i] = Y[i] + step_size * (23.0f*Yp[i] - 16.0f*AW[i * 2] + 5.0f*AW[i * 2 + 1]) / 12.0f;
        }
        // w(:,2) = w(:,1);
        for (uint i = 0; i < n; i++) {
            AW[i * 2 + 1] = AW[i * 2];
...
void Numerical::getY_next(float *Y) {
    memcpy(Y, Y_next, sizeof(float)*n);
}
